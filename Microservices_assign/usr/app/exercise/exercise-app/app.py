...
def get_photos():
    ...
    # print(tag, file=sys.stderr)         127.0.0.1:5000/listPhotos?albumName=alb1
    ...
    if albumName is not None:
        ...
        print('bruh',albumName)
        
        ph = []
        for p in Photo.objects(albums = albumName):
            ph.append( p)
        print(ph, file=sys.stderr)

    elif tag is not None:
        print('this is the query', tag, file=sys.stderr)
...
def add_album():
    ...
    output =jsonify( {'message': "Album successfully created", 'id':str( album.id)})
    #return output, 201
    return output, 201
    ##good
